...
from config import OPENAI_API_KEY, MODEL, MAX_RETRIES, RETRY_DELAY, BACKOFF_FACTOR
...
class OpenAIClient:
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError("APIキーが設定されていません。config.yamlを設定するか、環境変数OPENAI_API_KEYを設定してください。")
